...
public class SimpleEmailSender {
	...
	Session session = Session.getDefaultInstance(mailToSend.getProperties(), 
		new MyAuthenticator(mailToSend.getSenderAddress(), mailToSend.getSenderPassword()));
	try {
	    // 根据Session创建一个邮件消息
	    Message mailMessage = new MimeMessage(session);
	    
	    // 设置邮件发送者地址
	    mailMessage.setFrom(new InternetAddress(mailToSend.getSenderAddress()));
	    
	    // 设置邮件接受者类型和地址
	    mailMessage.setRecipient(Message.RecipientType.TO, new InternetAddress(mailToSend.getReceiverAddress()));
