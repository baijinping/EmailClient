...
public class EmailToSendInfo {
	...
	public void setSenderAddress(String senderAddress) {
		if (!senderAddress.matches(PanelConst.REGEX_EMAIL_ADDRESS)) {
			throw new IllegalArgumentException("Wrong Sender Email Address : "
					+ senderAddress);
		}
		this.senderAddress = senderAddress;
		this.senderHost = "smtp."
	...
	public void setReceiverAddress(String receiverAddress) {
		if (!receiverAddress.matches(PanelConst.REGEX_EMAIL_ADDRESS)) {
			throw new IllegalArgumentException(
					"Wrong Receiver Email Address : " + receiverAddress);
		}
		this.receiverAddress = receiverAddress;
		this.receiverHost = "smtp."
				+ receiverAddress.substring(receiverAddress.indexOf("@") + 1);
	}
